...
mod vector {
    ...
    impl Vector {
        ...
        pub fn vector_adding(&self, x: &Vector) -> Vector {
        ...
        pub fn scalar_adding(&self, x: &Vector) -> i32 {
            let mut sum = 0;
            for i in 0..VECTOR_LENGTH {
...
mod tests {
    ...
    fn empty_vector_adding() {
        let a = Vector::new();
        let b = a.vector_adding(&Vector::new());
    ...
    fn one_scalar_adding() {
        assert_eq!(0, Vector::new().scalar_adding(&Vector::new()));
        assert_eq!(3, Vector::init([1, 1, 1]).scalar_adding(&Vector::new()));
    }
}
